...
| Role | Definition |
|---|---|
| **Client** | The machine that sends a request for a service |
| **Server** | The machine that listens to the request and sends back a response |
...
|---|---|
| **LAN (Local Area Network)** | Client and server are on the same local network/router, communicating directly |
| **Internet / Public Network** | Used when client and server are on different networks — internet = interconnection of multiple networks |
